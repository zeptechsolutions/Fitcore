...
export const endpoints = {
  ...
  forgotPassword: (email) => api('/auth/password/forgot', { method: 'POST', body: JSON.stringify({ email }) }),
  verifyResetCode: (email, code) => api('/auth/password/verify-code', { method: 'POST', body: JSON.stringify({ email, code }) }),
  resetPassword: (email, code, password) => api('/auth/password/reset', { method: 'POST', body: JSON.stringify({ email, code, password }) }),
  me: () => api('/users/me'),
  updateMe: (payload) => api('/users/me', { method: 'PATCH', body: JSON.stringify(payload) }),
  changePassword: (payload) => api('/users/me/password', { method: 'PATCH', body: JSON.stringify(payload) }),
  bmi: () => api('/users/me/bmi'),
  personalPlan: () => api('/users/me/plan'),
  recalculatePlan: (applyToMacros = true) => api('/users/me/plan/recalculate', { method: 'POST', body: JSON.stringify({ applyToMacros }) }),
  ...
  addWater: (liters = 1) => api('/water', { method: 'POST', body: JSON.stringify({ liters }) }),
  subtractWater: (liters = 1) => api('/water/subtract', { method: 'POST', body: JSON.stringify({ liters }) }),
  deleteWater: (id) => api(`/water/${id}`, { method: 'DELETE' }),
  weights: () => api('/weights'),
  addWeight: (payload) => api('/weights', { method: 'POST', body: JSON.stringify(payload) }),
  deleteWeight: (id) => api(`/weights/${id}`, { method: 'DELETE' }),
  ...
  gymHistory: (limit = 30) => api(`/gym/history?limit=${limit}`),
  gymProgress: () => api('/gym/progress'),
  addGym: (payload) => api('/gym', { method: 'POST', body: JSON.stringify(payload) }),
  deleteGym: (id) => api(`/gym/${id}`, { method: 'DELETE' }),
  recipes: () => api('/recipes'),
  createRecipe: (payload) => api('/recipes', { method: 'POST', body: JSON.stringify(payload) }),
  analyzeRecipe: (payload) => api('/recipes/analyze', { method: 'POST', body: JSON.stringify(payload) }),
