...
function SavedMealBuilder({mode,onSaved}){
  const [name,setName]=useState(''),[servings,setServings]=useState(1),[defaultType,setDefaultType]=useState('snack'),[items,setItems]=useState([blankItem()]),[busy,setBusy]=useState(false),[error,setError]=useState('');
  ...
  const submit=async(e)=>{e.preventDefault();setBusy(true);setError('');try{const normalized=items.filter(x=>x.name.trim()).map(x=>({...x,quantity:Number(x.quantity)||1,calories:Number(x.calories)||0,protein:Number(x.protein)||0,carbs:Number(x.carbs)||0,fats:Number(x.fats)||0}));if(!normalized.length)throw new Error('Agregá al menos un ingrediente.');if(mode==='recipe')await endpoints.createRecipe({name,servings:Number(servings),ingredients:normalized});else await endpoints.createFavorite({name,defaultType,items:normalized});onSaved();}catch(e){setError(e.message)}finally{setBusy(false)}};
  return <form className="form-stack" onSubmit={submit}><div className="form-row"><label>Nombre<input value={name} onChange={e=>setName(e.target.value)} placeholder={mode==='recipe'?'Mi batido':'Mi desayuno'} required/></label>{mode==='recipe'?<label>Porciones<input type="number" min="1" value={servings} onChange={e=>setServings(e.target.value)}/></label>:<MealType value={defaultType} onChange={setDefaultType}/>}</div><div className="ingredient-builder"><div className="ingredient-head"><strong>{mode==='recipe'?'Ingredientes':'Alimentos'}</strong><button type="button" className="mini-add" onClick={add}><Plus size={15}/></button></div>{items.map((item,i)=><div className="ingredient-row" key={i}><div className="ingredient-main"><input value={item.name} onChange={e=>update(i,'name',e.target.value)} placeholder="Ingrediente"/><button type="button" onClick={()=>remove(i)} disabled={items.length===1}><X size={15}/></button></div><div className="ingredient-macros"><input type="number" value={item.calories} onChange={e=>update(i,'calories',e.target.value)} placeholder="kcal"/><input type="number" value={item.protein} onChange={e=>update(i,'protein',e.target.value)} placeholder="Prot"/><input type="number" value={item.carbs} onChange={e=>update(i,'carbs',e.target.value)} placeholder="Carb"/><input type="number" value={item.fats} onChange={e=>update(i,'fats',e.target.value)} placeholder="Grasa"/></div></div>)}</div><ErrorBox message={error}/><button className="btn primary" disabled={busy}>{mode==='recipe'?'Guardar receta':'Guardar favorito'}</button></form>
}
