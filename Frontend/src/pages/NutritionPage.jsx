...
function SavedMealBuilder({mode,onSaved}){
  ...
  if(mode==='recipe') return <form className="form-stack" onSubmit={submit}><div className="form-row"><label>Nombre<input value={name} onChange={e=>setName(e.target.value)} placeholder="Ingrese un nombre para la receta" required/></label><label>Porciones<input type="number" min="1" value={servings} onChange={e=>setServings(e.target.value)}/></label></div><label>Ingredientes y cantidades<textarea rows="5" value={description} onChange={e=>setDescription(e.target.value)} placeholder="Ingrese los ingredientes y cantidades de su receta"/></label><div className="recipe-ai-note"><Brain size={18}/><span><strong>Macros calculados con IA</strong><small>Al guardar, Zhealth analizará toda la receta y calculará calorías, proteína, carbos y grasas. Revisá siempre la estimación.</small></span></div><ErrorBox message={error}/>{aiInfo&&<p className="fineprint">{aiInfo.disclaimer}</p>}<button className="btn primary" disabled={busy}>{busy?'Analizando receta...':'Analizar y guardar receta'}</button></form>;
  return <form className="form-stack" onSubmit={submit}><div className="form-row"><label>Nombre<input value={name} onChange={e=>setName(e.target.value)} placeholder="Ingrese un nombre para el favorito" required/></label><MealType value={defaultType} onChange={setDefaultType}/></div><div className="ingredient-builder"><div className="ingredient-head"><strong>Alimentos</strong><button type="button" className="mini-add" onClick={add}><Plus size={15}/></button></div>{items.map((item,i)=><div className="ingredient-row" key={i}><div className="ingredient-main"><input value={item.name} onChange={e=>update(i,'name',e.target.value)} placeholder="Ingrese el alimento"/><button type="button" onClick={()=>remove(i)} disabled={items.length===1}><X size={15}/></button></div><div className="ingredient-macros"><input type="number" value={item.calories} onChange={e=>update(i,'calories',e.target.value)} placeholder="kcal"/><input type="number" value={item.protein} onChange={e=>update(i,'protein',e.target.value)} placeholder="Proteína"/><input type="number" value={item.carbs} onChange={e=>update(i,'carbs',e.target.value)} placeholder="Carbos"/><input type="number" value={item.fats} onChange={e=>update(i,'fats',e.target.value)} placeholder="Grasas"/></div></div>)}</div><ErrorBox message={error}/><button className="btn primary" disabled={busy}>Guardar favorito</button></form>
}
