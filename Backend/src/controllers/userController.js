...
export async function updateMe(req, res) {
  const allowed = ['name','birthDate','heightCm','startingWeightKg','targetWeightKg','goal','activityLevel','macroGoals','waterGoalLiters','bottleSizeLiters','weeklyGymGoal','privacy'];
  const changes = {};
...
export async function getBmi(req, res) {
  ...
  const weight = latest?.weightKg ?? user.startingWeightKg;
